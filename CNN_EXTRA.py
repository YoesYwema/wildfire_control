...
class CNN_EXTRA:
    ...
    def train(self):
        ...
        train_extra_path = "train_extra/" + environment + size
        train_states_batch = list()
        train_action_batch = list()
        train_batch = list()
        # Validation variables
        validation_path = "validate/" + environment + size
        validation_extra_path = "validate_extra/" + environment + size
        ...
        train_extra_memories = len([name for name in os.listdir(train_extra_path) if os.path.isfile(os.path.join(train_extra_path, name))])
        print("\n[train_extra_memories]", train_extra_memories)
        validation_extra_memories = len([name for name in os.listdir(validation_extra_path) if os.path.isfile(os.path.join(validation_extra_path, name))])
        print("[validation_extra_memories]", validation_extra_memories)
        ...
        i = 0
        # Extract extra train memories
        while i < train_extra_memories:
            x = extract_from_memory(train_extra_path)
            train_batch.append(x)
            i = i + 1
        ...
        i = 0
        # Extract extra validation memories
        while i < validation_extra_memories:
            x = extract_from_memory(validation_extra_path)
            validation_batch.append(x)
            i = i + 1
    ...
    # Store an experience in /train_EXTRA/environment/sizeX.
    def remember(self, state, action, train, done):
        ...
        if train:
            ...
            if not os.path.exists("train_extra/" + path):
                os.makedirs("train_extra/" + path)
            # add counter to the end of the name of files
            counter = 0
            while os.path.isfile("train_extra/" + path + name):
                if counter > 0:
            ...
            with open("train_extra/" + path + name, "wb") as pf:
                pickle.dump((state, action, done), pf)
                pf.close()

        # VALIDATION DATA
        if not train:
            if not os.path.exists("validate_extra/" + path):
                os.makedirs("validate_extra/" + path)
            # add counter to the end of the name of files
            counter = 0
            while os.path.isfile("validate_extra/" + path + name):
                if counter > 0:
                    n_digits_to_delete = len(str(counter))
                    name = name[:-n_digits_to_delete]
                name = name + str(counter)
                counter += 1
            with open("validate_extra/" + path + name, "wb") as pf:
                pickle.dump((state, action, done), pf)
                pf.close()
    ...
    def write_logs(self):
        ...
        if not os.path.exists("Logs_EXTRA/"):
            os.makedirs("Logs_EXTRA/")
        # Write all logs into one file for each sort of simulation
        with open("Logs_EXTRA/" + name, 'ab+') as f:
